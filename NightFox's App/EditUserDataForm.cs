...
namespace test
{
    public partial class EditUserDataForm : Form
    {
        ...
        private void inputUsername__TextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            // Личная информация
            SQLData.command = new SqlCommand("SELECT TOP 1 account_id, nickname, password, email, COALESCE(photo_ext, '-'), isAdmin FROM accounts WHERE nickname LIKE '" +
                inputUsername__TextBox.Text + "%'", SQLData.connection);
